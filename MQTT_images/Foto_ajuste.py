...
def capturar_imagem(caminho_imagem):
    try:
        ...
        # Configurar ajustes da câmera
        config = picam2.create_still_configuration()
        picam2.configure(config)
        
        # Ajustar balanço de branco automático
        picam2.set_controls({
            "AwbEnable": True,  # Ativa o Auto White Balance
            "Brightness": 0.5,  # Ajuste de brilho (opcional, varia entre 0 e 1)
            "Contrast": 1.2,    # Contraste (pode ser ajustado)
            "Saturation": 1.5,  # Saturação para cores mais vivas
            "Sharpness": 1.0    # Nitidez
        })
